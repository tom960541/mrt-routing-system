...
MAP_DATABASE = {
    "高雄捷運": {"json": "krt_data.json", "img": "krt_map.jpg"},
    "台北捷運": {"json": "tpi_data.json", "img": "TaipeiMetroStamp.png"}
}
...
def get_stations_from_ai(user_text, system):
    try:
        ...
        prompt = f"""
        你是一個智慧捷運站點解析器。請從使用者的句子中推斷起點與終點捷運站。
        這是一個動態載入的地圖，可用站點列表：[{station_info}]
        如果說的是地標，請轉換成最近的捷運站代號。
        請「絕對嚴格」只輸出 JSON 格式。
        範例：{{"start_id": "R16", "end_id": "O10"}}
        使用者輸入：「{user_text}」
        """
...
def calculate_fare_details(system, path_ids):
    ...
    for i in range(1, len(path_ids)):
        ...
        if next_line != curr_line:
            ...
            fare = min(20 + (math.ceil((dist-5)/2)*5) if dist>5 else 20, 35 if "C" in curr_line or "LRT" in curr_line else 60)
            total_fare += fare
            details.append(f"- {curr_line}: {system.get_station(segment[0]).name} -> {system.get_station(segment[-1]).name} (票價 {fare} 元)")
...
# 初始化 Session State (紀錄 AI 分析與點擊狀態)
if 'ai_start' not in st.session_state: st.session_state.ai_start = None
if 'ai_end' not in st.session_state: st.session_state.ai_end = None
...
map_files = MAP_DATABASE[selected_map_name]

# 載入系統資料
system_data = load_system_data(map_files["json"])
krt = TransitSystem(system_data)

if not system_data:
    st.error(f"找不到地圖資料檔：{map_files['json']}，請確定檔案存在同一目錄下。")
    st.stop()
...
with col_ui:
    ...
    if st.button("🤖 AI 幫我找"):
        with st.spinner('AI 正在努力分析您的需求...'):
            ...
            if s_id and e_id and krt.get_station(s_id) and krt.get_station(e_id):
                st.session_state.ai_start = krt.get_station(s_id).display_name
                st.session_state.ai_end = krt.get_station(e_id).display_name
    ...
    start_idx = display_names.index(st.session_state.ai_start) if st.session_state.ai_start in display_names else 0
    end_idx = display_names.index(st.session_state.ai_end) if st.session_state.ai_end in display_names else 0
    
    start_station = st.selectbox("出發站", display_names, index=start_idx)
    end_station = st.selectbox("目的站", display_names, index=end_idx)

    # 4. 執行查詢
    if st.button("🔍 查詢最省錢路徑", type="primary"):
...
with col_map:
    ...
    try:
        img = Image.open(map_files["img"])
        
        # 使用 streamlit_image_coordinates 替代 st.image
        ...
        if click_coords is not None:
            ...
            if st.session_state.last_click != click_point:
                ...
                # 若點擊位置在站點半徑 80 像素內，則判定選中
                if closest_station and min_dist < 80:
                    if st.session_state.next_click_is_start:
                        st.session_state.ai_start = closest_station.display_name
                        st.session_state.next_click_is_start = False
                    else:
                        st.session_state.ai_end = closest_station.display_name
