...
# 系統設定與地圖預設資料庫
# ==========================================
# 提醒：為了解決先前的編碼問題，這裡暫時將金鑰寫死。
# 未來若要上傳公開的 GitHub，請務必改用 st.secrets 隱藏金鑰！
...
class Station:
    def __init__(self, sid, name, coords, line_type, neighbors):
        self.sid = sid
        self.name = name
        # 捨棄代號前綴，直接用純中文站名作為 UI 顯示，方便前端合併重複站名
...
if 'start_station' not in st.session_state:
    st.session_state.start_station = display_names[0] if display_names else ""
if 'end_station' not in st.session_state:
    st.session_state.end_station = display_names[0] if display_names else ""
if 'next_click_is_start' not in st.session_state: 
...
with col_map:
    ...
    try:
        ...
        if click_coords is not None:
            ...
            # 開發者除錯儀表板
            st.caption(f"🔧 **工程師除錯資訊** | 您的點擊座標：(X: {cx}, Y: {cy}) | 最近站點：{closest_station.name if closest_station else '無'} | 距離：{min_dist:.1f} 像素")
